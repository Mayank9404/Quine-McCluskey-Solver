def change4(dec,bin):
    #bin = ['0,1,2,3', '3,7,11,15']
    #dec = [{0: ['000-', '00-0'], 1: ['00-1', '001-'], 2: ['0-11', '-011'], 3: ['-111', '1-11']}, {0: ['00--'], 2: ['--11']}]
    #out: [{1: {'0001': 1, '0010': 2, '0100': 4, '1000': 8}}, {2: {'1001': 9}}, {3: {'0111': 7, '1101': 13}}]}
    new = []
    if len(dec) >= 2:
        i = dec[1]
        key = list(i.keys())
...
def change3(dec,bin):
    #bin = ['2,6', '4,6', '6,7', '6,14', '7,15', '11,15', '14,15']
    #dec = [{0: ['0-10', '01-0'], 1: ['011-', '-110'], 2: ['-111', '1-11', '111-']}, {1: ['-11-']}]
    #out: [{1: {'0001': 1, '0010': 2, '0100': 4, '1000': 8}}, {2: {'1001': 9}}, {3: {'0111': 7, '1101': 13}}]}
    new = []
    if dec:
        i = dec[0]
...
def change2(old):
    new = []
    for i in old:
        #[[1, {'0001': 1, '0010': 2, '0100': 4, '1000': 8}], [2, {'1001': 9}], [3, {'0111': 7, '1101': 13}]]}
        temp = {}
        temp[i[0]] = i[1]
        new.append(temp)    
    return new
...
def all_prime(prime1):
    sol = ', '.join(prime1)
    return sol

def prime_table(fmin,mt):
    d = {}
    fmin2= []
    for i in fmin:
        fmin2.append(list(map(int,i.split(','))))
    for j in range(len(fmin2)):
        temp = list()
        for i in mt:
            if i in fmin2[j]:
                temp.append(1)
...
def mains(mt,dc=' '):
    ...
    while True:
        ...
        if should_stop: # If the minterms cannot be combined further
            ...
            all_prime_imp = all_prime(list(all_pi))
            break
        #print('prime',prime)
        #all_prime(prime)
    ...
    #print(second,'\n\n')
    #print(dash)
    second_group = change3(dash,second)
    third_group = change4(dash,third)
    # Printing and processing of Prime Implicant chart starts
    sz = len(str(mt[-1])) # The number of digits of the largest minterm
    chart = {}
    x = ' '.join((' '*(sz-len(str(i))))+str(i) for i in mt)
    prime_header = [z for z in x.split() ]
    #print(prime_header)
    ...
    '''print(final_min)
    print(l)'''
    prime_t = prime_table(final_min,mt)
    ...
    essential = ', '.join(str(i) for i in EPI)
    ...
    solu = ' + '.join(''.join(i) for i in final_result)
    return first_group,second_group,third_group,all_prime_imp,prime_header,final_min,prime_t,essential,solu
